...
use codex_tools::mcp_tool_to_freeform_tool;
...
impl McpHandler {
    pub fn new(tool_info: ToolInfo) -> Result<Self, serde_json::Error> {
        let freeform = tool_info.mcp_freeform && is_exact_freeform_input_schema(&tool_info.tool);
        let tool_name = if freeform {
            ToolName::plain(join_tool_name(&tool_info.canonical_tool_name()))
        } else {
            tool_info.canonical_tool_name()
        };
        let spec = create_tool_spec(&tool_info, &tool_name, freeform)?;
        Ok(Self {
            tool_info,
            tool_name,
            freeform,
    ...
    #[cfg(test)]
    pub fn new_freeform(mut tool_info: ToolInfo) -> Result<Self, serde_json::Error> {
        tool_info.mcp_freeform = true;
        let tool_name = ToolName::plain(join_tool_name(&tool_info.canonical_tool_name()));
        let spec = create_tool_spec(&tool_info, &tool_name, /*freeform*/ true)?;
        Ok(Self {
            tool_info,
            tool_name,
            freeform: true,
...
fn create_tool_spec(
    ...
    if freeform {
        return Ok(ToolSpec::Freeform(mcp_tool_to_freeform_tool(
            tool_name,
            &tool_info.tool,
        )?));
    }
...
mod tests {
    ...
    async fn mcp_freeform_pre_tool_use_payload_wraps_raw_input() {
        ...
        let handler = McpHandler::new_freeform(tool_info("patches", "mcp__patches__", "apply"))
            .expect("MCP freeform tool spec should build");
