        ...
        for (namespace, mut entries) in namespace_entries {
            ...
            for (tool_name, tool) in entries {
                if tool.mcp_freeform && is_exact_freeform_input_schema(&tool.tool) {
                    match mcp_tool_to_freeform_tool(&tool_name, &tool.tool) {
                        Ok(converted_tool) => {
                            builder.push_spec(ToolSpec::Freeform(converted_tool));
                            builder
                                .register_handler(Arc::new(McpHandler::new_freeform(tool.clone())));
                            continue;
                        }
                        Err(error) => {
                            tracing::error!(
