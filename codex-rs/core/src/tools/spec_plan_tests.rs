...
fn direct_mcp_freeform_tool_is_exposed_as_top_level_freeform_tool_when_opted_in() {
    ...
    let mut freeform_info = tool_info_from_parts(
        &ToolName::namespaced("mcp__sample__", "apply_patch"),
        freeform_tool,
    );
    freeform_info.mcp_freeform = true;
    let mut echo_info =
        tool_info_from_parts(&ToolName::namespaced("mcp__sample__", "echo"), echo_tool);
    echo_info.mcp_freeform = true;

    let builder = build_tool_registry_builder(
        &tools_config,
        ToolRegistryBuildParams {
            mcp_tools: Some(&[freeform_info, echo_info]),
            deferred_mcp_tools: None,
    ...
    let (tools, registry) = builder.build();
    ...
    assert!(registry.has_handler(&ToolName::plain("mcp__sample__apply_patch")));
    assert!(registry.has_handler(&ToolName::namespaced("mcp__sample__", "echo")));
...
fn non_exact_mcp_freeform_schema_stays_namespaced_function_tool() {
    ...
    let mut rewrite_info = tool_info_from_parts(
        &ToolName::namespaced("mcp__sample__", "rewrite"),
        rewrite_tool,
    );
    rewrite_info.mcp_freeform = true;

    let builder = build_tool_registry_builder(
        &tools_config,
        ToolRegistryBuildParams {
            mcp_tools: Some(&[rewrite_info]),
            deferred_mcp_tools: None,
            tool_namespaces: None,
            discoverable_tools: None,
            extension_tool_bundles: &[],
    ...
    find_namespace_function_tool(&tools, "mcp__sample__", "rewrite");
    assert!(registry.has_handler(&ToolName::namespaced("mcp__sample__", "rewrite")));
    assert_lacks_tool_name(&tools, "mcp__sample__rewrite");
}
