...
fn mcp_freeform_tool(
    ...
    let mut tool = mcp_tool(server, namespace, name);
    tool.mcp_freeform = true;
    tool.tool.input_schema = Arc::new(rmcp::model::object(input_schema));
    tool
}
...
#[tokio::test]
async fn direct_mcp_freeform_tool_is_exposed_as_top_level_freeform_tool_when_opted_in() {
    let freeform_tool = mcp_freeform_tool(
        "sample",
        "mcp__sample",
        "apply_patch",
        json!({
    ...
    let echo_tool = mcp_freeform_tool(
        "sample",
        "mcp__sample",
        "echo",
        json!({
    ...
    let plan = probe_with(
        |_| {},
        ToolPlanInputs {
            mcp_tools: Some(vec![freeform_tool, echo_tool]),
            ..ToolPlanInputs::default()
        },
    )
    .await;

    let ToolSpec::Freeform(tool) = plan.visible_spec("mcp__sample__apply_patch") else {
        panic!("expected exact freeform MCP tool to be exposed as a top-level custom tool");
    };
    assert_eq!(tool.name, "mcp__sample__apply_patch");
    assert_eq!(
        tool.description,
        "apply_patch test tool\n\nInput:\nRaw patch program text."
    );
    assert_eq!(
        plan.namespace_function_names("mcp__sample"),
        &["echo".to_string()]
    );
    plan.assert_registered_contains(&[
        "mcp__sample__apply_patch",
        &ToolName::namespaced("mcp__sample", "echo").to_string(),
    ]);
}

#[tokio::test]
async fn non_exact_mcp_freeform_schema_stays_namespaced_function_tool() {
    let rewrite_tool = mcp_freeform_tool(
        "sample",
        "mcp__sample__",
        "rewrite",
        json!({
    ...
    let plan = probe_with(
        |_| {},
        ToolPlanInputs {
            mcp_tools: Some(vec![rewrite_tool]),
            ..ToolPlanInputs::default()
        },
    )
    .await;

    plan.assert_visible_lacks(&["mcp__sample__rewrite"]);
    assert_eq!(
        plan.namespace_function_names("mcp__sample__"),
        &["rewrite".to_string()]
    );
    plan.assert_registered_contains(&[
        &ToolName::namespaced("mcp__sample__", "rewrite").to_string()
    ]);
}

#[tokio::test]
async fn deferred_mcp_freeform_tool_is_discoverable_but_not_exposed_as_custom_tool() {
    let freeform_tool = mcp_freeform_tool(
        "sample",
        "mcp__sample__",
        "apply_patch",
    ...
    let plan = probe_with(
        ...
        ToolPlanInputs {
            deferred_mcp_tools: Some(vec![freeform_tool]),
            ..ToolPlanInputs::default()
        },
    )
    .await;
    ...
    plan.assert_visible_contains(&["tool_search"]);
    plan.assert_visible_lacks(&["mcp__sample__apply_patch"]);
    plan.assert_registered_contains(&[
        &ToolName::namespaced("mcp__sample__", "apply_patch").to_string()
    ]);
    assert_eq!(
        plan.exposure(&ToolName::namespaced("mcp__sample__", "apply_patch").to_string()),
        ToolExposure::Deferred
    );
}
